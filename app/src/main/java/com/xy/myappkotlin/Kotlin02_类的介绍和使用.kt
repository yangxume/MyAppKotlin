 ...
 * 二.定义一个类：
            用class关键字来声明类。
            类的声明形式：
            1.class 类名<某个类型> constructor (参数名：参数类型).
                    例如：class Test<T> constructor(str:T){ }
            2.不指定某个类型的类，可以省略<T>。
                    例如：class Test constructor(str:String){}
            3.不指定类的主构造方法,可以省略constructor(str:T)。
                    例如： class Test{}
 * */
 ...
 * 3.直接构造方法中声明全局变量
 */
class BianLiang1<T> constructor(str:T){

    var bianliang : T

    var bianling2 : Int = 33
    ...
    fun test(){
        println("bianliang2 = "+bianling2)
    }
}

class Bianliang2 constructor(val str:String){
...
/**
 * 构造方法私有化:用private关键字修饰一个空的主构造方法
 */
